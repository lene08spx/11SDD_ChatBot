...
export class IBPubBot {
	...
	public async run (): Promise<void> {
		...
		//================================
		let menuQuery: PBMenuViewRecord[] = await this._db.query("SELECT * FROM MenuView;");
		this._menu = new PBMenu(menuQuery);
...
//            if (this._ui.intent(userAnswer,PBFuzzy.MATCH_QUIT),80) resolve(null);
	...
	private _getNonFuzzyWords(str: string, fuzzyArray: string[], threshold = 80) {
		//let thresh = threshold;
		return str.split(" ").filter(v=>{
			let res = false;
			for (let fuzzWord of fuzzyArray) {
				for (let f=0;f<fuzzWord.split(" ").length;f++) {
					if(this._ui.intent(v,[fuzzWord.split(" ").slice(0,fuzzWord.split(" ").length-f).join(" ")],threshold)) res = true;
					if(this._ui.intent(v,[fuzzWord.split(" ").slice(f).join(" ")],threshold)) res = true;
				}
			}
			return !res;
		});
	...
	private async _printOrders(orders: PBOrder[], previewOrder=false) {
		for (let i=0; i<orders.length; i++) {
			...
			if (this._ui.ttsEnabled) await this._ui.sleep(1500);
			this._ui.div("-");
	...
	private async _printMenu() {
		if (!this._menu) return;
		this._ui.div();
		await this._ui.print("Here is the menu: Costs in $AUD");
		this._ui.div("-");
		if (this._menu.main.length>0) await this._ui.print("Mains:");
		for (let item of this._menu.main) {
			await this._ui.print("   "+item.name+"                        ".slice(item.name.length)+("$"+String(item.cost)).padStart(5," "));
		}
		if (this._menu.main.length>0) await this._ui.print("Drinks:");
		for (let item of this._menu.drink) {
			await this._ui.print("   "+item.name+"                        ".slice(item.name.length)+("$"+String(item.cost)).padStart(5," "));
		}
		if (this._menu.main.length>0) await this._ui.print("Desserts:");
		for (let item of this._menu.dessert) {
			await this._ui.print("   "+item.name+"                        ".slice(item.name.length)+("$"+String(item.cost)).padStart(5," "));
		}
	}

	private _userWantsMenu(): Promise<boolean> {
		return new Promise(async resolve=>{
			let thresh = 80;
			let userAnswer = (await this._ui.input(
				"Would you like to see the menu?"
			));
			let userIntent = (	this._ui.intent(userAnswer,PBFuzzy.MATCH_YES,thresh) 
								|| this._stringIncludesTextFromArray(userAnswer,PBFuzzy.MATCH_YES,70)
								|| this._ui.intent(userAnswer,PBFuzzy.MATCH_MENU,thresh));
			if (!userIntent) {
				userIntent = this._ui.intent(userAnswer,PBFuzzy.MATCH_NO,thresh);
				if (userIntent) {
					resolve(false);
				} else {
					await this._ui.print(
	...
	private async _getUserOrderName (): Promise<string> {
	...
	private _getYesNo (question: string, threshold = 60): Promise<boolean> {
		return new Promise(async resolve=>{
			let tries = 2;
			let userAnswer = "";
			...
			while (tries > 0) {
				...
				userIntent = this._ui.intent(userAnswer,PBFuzzy.MATCH_YES,threshold) || this._stringIncludesTextFromArray(userAnswer,PBFuzzy.MATCH_YES,70);
				if (!userIntent) {
					userIntent = this._ui.intent(userAnswer,PBFuzzy.MATCH_NO,threshold);
					if (userIntent) {
						resolve(false);
						break;
					} else {
