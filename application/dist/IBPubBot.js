    ...
    async _printOrders(orders, previewOrder = false) {
        for (let i = 0; i < orders.length; i++) {
            ...
            if (previewOrder)
                ...
                await this._ui.print("Order ID. " + String(orders[i].orderID));
            if (!previewOrder)
        ...
        return new Promise(async (resolve) => {
            ...
            if (this._ui.intent(userAnswer, PBFoundation_1.PBFuzzy.MATCH_MENU, thresh)) {
                ...
                if (userOrderName === "" && userAnswer === "") {
                    ...
                    if (this._getNonFuzzyWords(userAnswer, PBFoundation_1.PBFuzzy.MATCH_ORDER).join(" ").toLowerCase() === userAnswer) {
                        resolve("@!PREFIX");
                    }
                    else {
